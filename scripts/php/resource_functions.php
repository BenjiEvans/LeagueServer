...
if(!is_team_full($name)){
  ...
  if(has_sent_request($name)) echo "<span id='$iden' ><button disabled type='button' class='btn btn-success team_rank_btn join' style='color:rgb(0,0,0)'><img src='/img/glyphicons_006_user_add.png'> <span>Join Request Sent</span></button></span>";
...
$captain = $row['captain'];
$q->close();
$query = $mysqli->query("select count(*) as total from Notes where sender=$id and type=1 and recipient=$captain");
